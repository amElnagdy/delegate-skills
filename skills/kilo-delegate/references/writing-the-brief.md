...
A brief is the entire task as Kilo will see it. Kilo runs in a fresh session with **no memory of
...
Kilo. The single most common failure is a brief that assumes context Kilo doesn't have.

## Match the model to the brief

Kilo has a configured default model, so a fresh dispatch may omit `--model`. Passing `provider/model`
is still the way to pin a catalog id (including ids with `~`). Which
...
- **The allowed set is the human's to state.** `kilo models` lists a few hundred models, but most
  ...
  their `CLAUDE.md` (e.g. `kilo-go/…`, `zai-coding-plan/…`, `minimax-coding-plan/…`). If they
...
Kilo responds well to compact, block-structured prompts with XML tags rather than long prose. State
...
change, and explicitly what to leave untouched. The "leave untouched" list is what keeps Kilo from
...
- **Review / diagnosis (read-only)** — add `<grounding_rules>` (ground every claim in evidence; label
  inferences) and dispatch with `--read-only` so Kilo runs as the `plan` agent and can't edit.
- **Research / recommendations** — add `<research_mode>` (separate observed facts, inferences, open
  questions).

## Always ask for the report explicitly

The relay assembles Kilo's final message from the text it emits when it stops. If the agent finishes
...
report you can read: it tells Kilo to end with a written summary, so the result file carries one.
...
naming them gets you a Kilo that guesses — or skips.

## Honor the repo's conventions

Kilo reads the repo's `AGENTS.md` automatically, so house rules there (style, forbidden patterns,
...
repo's own conventions ban — restate the load-bearing ones in the brief too, because Kilo's
...
a roadmap" produces a muddled run; split it into separate dispatches. One brief → one Kilo run →
