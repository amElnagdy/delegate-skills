...
`scripts/relay.mjs` is the dispatch layer. It wraps `kilo run`, runs the brief under the chosen
agent, captures everything, and writes a structured `result.json`. Your job collapses to: run one
command, then read one file. Everything Kilo-specific lives in the helper, which is what keeps the
...
```bash
command -v kilo    # the active binary on PATH
kilo --version     # the relay records this in result.json too
kilo auth list     # at least one provider credential must be present
```
...
| `--model <name>` | Model as `provider/model`. Optional — Kilo has a configured default; a resumed run inherits its session's model. Ids may include `~`. |
| `--agent <name>` | Kilo agent (default: `code`, write-capable). |
...
| `--no-auto` | The relay passes `kilo`'s `--auto` (auto-approve permissions) **by default** so a headless run doesn't hang on a prompt; `--no-auto` drops it and honors the agent's own permission config instead. A `--read-only`/`plan` run never gets `--auto`, so it can't be auto-approved into edits. |
| `--resume-last` | Continue the most recent Kilo session; send only the delta brief (see review-and-land). |
...
touched-files report shows only Kilo's edits and nothing of the helper's own.
...
- `tool` — `kilo`
- `status` — `completed` | `failed` | `timeout` | `aborted` | `kilo_unavailable`
- `exitCode` — mirrors Kilo's exit code; `128` plus the signal number if the child was killed; `127` if `kilo` isn't on PATH; on a `timeout` the relay forces a non-zero code even when the child exited `0` after the watchdog's SIGTERM
- `signal` — the signal that killed the child, otherwise `null`
- `kiloVersion` — the binary that actually ran
- `agent` — the agent selected for this dispatch (`code`, `plan`, …)
- `sessionId` — feed this to a later `--session <id>` (or use `--resume-last`)
- `finalMessage` — Kilo's assembled final text (the `<structured_output_contract>` you asked for).
  Empty if Kilo stopped without emitting a closing summary — ask for the report explicitly
...
- `stderrTail` — last ~20 stderr lines; present on every run that did not complete (`failed`, `timeout`, `aborted`), absent on `completed`,
  `kilo_unavailable`, and launch failures
- `error` — present on a launch failure, and on `timeout` and `aborted` runs

The helper also prints a summary to stdout and exits with Kilo's exit code, so a wrapping script can
branch on success/failure directly.

## Waiting for completion

The helper blocks until Kilo finishes. Back it with whatever your orchestrator offers:
...
- **Plain shell / other agents:** foreground for short tasks, or background and poll — `node relay.mjs
  ...
  too, don't only watch for the file. (A missing `kilo` binary exits 127 but *does* write a
...
- **`status: kilo_unavailable` (exit 127):** `kilo` isn't on PATH or isn't found. Install from
  kilo.ai and `kilo auth login`, then re-dispatch.
- **an `error` mentioning `version preflight` (`failed`, or `timeout` at exit 124):** the bounded
  `kilo --version` probe exited non-zero or hung past its cap (10s, or `--timeout` when shorter),
  so kilo was never dispatched; only the relay's own artifacts may already exist under
  `--out-dir`. Check the install by running `kilo --version` yourself.
...
- **`status: aborted`:** the relay itself was killed (its parent's timeout, a stopped task, a
  closed terminal) and forwarded the kill to kilo. The result is written before the relay exits;
...
- **Empty `finalMessage`:** Kilo finished without emitting a closing text summary (common when it
...
```bash
kilo run --format json --dir <cd> --agent code --auto < brief.txt   # fresh run (model optional)
kilo run --format json --dir <cd> --continue --agent code --auto < delta-brief.txt
kilo run --format json --dir <cd> --session ses_… --agent code --auto < delta-brief.txt
```
...
`finalMessage` from the `text` events and pulls `sessionId` from the event stream. Kilo selects an
agent for each prompt, so the helper passes the requested agent on fresh and resumed runs.

If you ever want it, raw `kilo run` is fine for one-offs — you just give up the captured
`result.json`, touched-files summary, and session-id extraction the helper does for you.

## The commit boundary

The helper never commits — by design, not omission. The robust contract is: Kilo edits the working
