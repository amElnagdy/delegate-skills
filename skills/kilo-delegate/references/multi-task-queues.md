...
`relay.mjs` dispatch starts a new Kilo session, so independent tasks don't share context — fold any
...
task's brief** as an explicit line. A fresh Kilo session has no memory of the earlier run, so a
...
- **"Needs your eyes"** — design decisions Kilo made, non-blocking nitpicks, anything you want the
