...
Kilo did the typing; you own the judgment. This is where delegation earns its keep or quietly ships
...
`result.json` carries Kilo's own claim that the gates passed. Treat that as a claim, not evidence —
...
- **Scope creep** — did Kilo change things the brief said to leave untouched? Unasked refactors,
...
- **Quiet judgment calls** — sometimes Kilo makes a defensible decision the brief didn't anticipate.
...
- **Hardcoded success or fixture data** on a path the brief says does real work — a canned
  `{status: "ok"}` or default return passes tests *by design*. If Kilo couldn't implement something,
...
Anything the sweep catches goes back to Kilo as a delta brief (below) or gets fixed in the tree
...
`code` agent *can* write the working tree, but committing should be the act of the party that verified
...
If the review turns up problems, don't restate the entire brief. Continue the same Kilo session with
just the correction:
...
`--resume-last` keeps Kilo's session context from the first run (and its model), so a short delta is
...
- **Report design decisions** Kilo made, and any defensible-but-unrequested turns it took.
...
- **Stop and ask** if correct completion requires going beyond the brief — don't expand the mandate on
  your own. A scope change is the human's call, not yours or Kilo's.
