...
name: kilo-delegate
description: >-
  Delegate a coding task to the Kilo CLI as a background implementer, then review its diff and
  land it yourself. Use this whenever the user wants to hand implementation work to Kilo — phrasings
  like "have Kilo do X", "delegate this to Kilo", "run it through Kilo", or "use Kilo to
  implement/fix/refactor" — or wants to run a queue of coding tasks through Kilo while staying the
...
license: MIT
compatibility: Requires the `kilo` CLI installed and authenticated (`kilo auth login`), Node 18+, and git. The orchestrating agent must be able to run shell commands and read files. Shell examples assume bash/zsh (macOS/Linux, or Git Bash/WSL on Windows).
metadata:
  version: 0.5.0
---
...
**implementer** — the Kilo CLI — then review what it produced and land it yourself. You write
the brief and own the judgment; Kilo does the typing in its own session; you verify and commit.

Nothing here is specific to one orchestrating agent. The loop needs only the ability to run a shell
command and read a file, so any agent with those two capabilities — Claude Code, Kilo driving a
...
- The `kilo` CLI is not installed or not authenticated (run `kilo auth login`).
- You want to write the code yourself, or you only need a review (use the `plan` agent via `--read-only`).

## Prerequisites (check once)

1. `kilo --version` succeeds. If not, install from kilo.ai and run `kilo auth login`.
2. **Confirm which `kilo` is on PATH.** `command -v kilo` shows the active binary and
   `kilo --version` its version. The relay records the version it ran into `result.json`, so a stale
   binary is visible after the fact.
3. A model provider is authenticated — `kilo auth list` shows at least one credential.
4. You are in (or will point `--cd` at) the target git repository.

## Choose the implementer model

Kilo has a configured default model, so a fresh run may omit `--model`. Pass one via `--model` or a
fleet `--lane` that sets one; a resumed run inherits its session's model. Naming the model is the one
decision a single-model backend like codex-delegate never had, and it has two owners:

- **The human owns which models are allowed.** `kilo models` lists hundreds of entries, most billed
  ...
  `AGENTS.md` or their `CLAUDE.md` (e.g. "delegate mechanical work to `kilo-go/…`, hard logic to
  `…`").
...
Kilo sees **only** the text you send plus what it can read from the working tree — no chat history,
...
AGENTS.md/CLAUDE.md/Makefile — do not assume), and a report contract. Tell Kilo it will **not**
commit (you will). Keep one task per brief. Full guidance and a template:
[references/writing-the-brief.md](references/writing-the-brief.md).

### 2. Dispatch

Send the brief to Kilo with the bundled helper. It wraps `kilo run`, captures the run, and
...
directory — if unsure where it landed, run `find ~ -name relay.mjs -path '*kilo-delegate*'` and
substitute the directory above it.)

```bash
node "<skill-dir>/scripts/relay.mjs" --brief brief.txt --cd /path/to/repo
# optional model:                           add --model provider/model  (ids may include ~)
...
# continue the previous Kilo session:   add --resume-last  (delta brief only; keeps the model)
# hard time limit (watchdog):               add --timeout 2h  (default: off; implementation runs routinely need 1-2h)
# see all options:                          node .../relay.mjs --help
```

The helper defaults to the write-capable `code` agent and writes its artifacts to a temp dir, so the
...
The helper blocks until Kilo finishes, so back it with whatever your orchestrator offers and resume
when it returns:
...
- **Plain shell / other agents:** run it in the foreground for short tasks, or background it and poll
  ...
  file, so check the exit code too. A missing `kilo` binary exits 127 but *does* write a
  `result.json` with status `kilo_unavailable`.)
...
Kilo's `result.json` includes its own final message and any gate claims. **Re-verify, don't accept:**

- **Re-run the project's gates yourself** (the test/lint/build commands from step 1). Never take
  "gates passed" on faith.
- **Read the diff** against the brief: did Kilo do what was asked, nothing more (scope creep) and
...
Kilo's autonomy is governed by the **agent**, not a sandbox enum:

- **`code`** (the relay default) — write-capable; edits files in the working dir headlessly. The
...
mandate: **surface, don't absorb** (report Kilo's design decisions, defensible-but-unasked turns,
...
- [references/writing-the-brief.md](references/writing-the-brief.md) — how to write a brief Kilo can
