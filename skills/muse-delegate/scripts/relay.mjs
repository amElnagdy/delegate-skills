#!/usr/bin/env node
/**
 * delegate-skills · muse-delegate · relay.mjs
 *
 ...
 * only). It shells out only to `muse` and `git`. The `muse` process it launches
 ...
 *
 * Headless Muse is `muse exec` only. `muse resume` is the interactive TUI and is
 * never invoked. Write runs keep Muse's sandbox on and pass `--disable-approval`
 * so a headless run cannot hang on a tool-approval prompt. `--read-only` adds
 * `--disable-write` (non-shell filesystem writes) and `--disable-shell`.
 * `--yolo` is not the default and is not exposed.
 *
 * Usage:
 ...
 *   --cd <dir>              Working root for Muse (default: current directory).
 *   --lane <name>           Fleet lane from delegate-setup config (dials apply; explicit flags win).
 *   --model <name>          Muse model id (optional; Muse's own default applies).
 *   --effort <level>        Passed as --reasoning-effort: none|minimal|low|medium|high|xhigh|ultra.
 *   --provider <mode>       Relay-only: echo | meta. Omit to use Muse's default (meta). Not a fleet dial.
 *   --read-only             Add --disable-write and --disable-shell (keep --disable-approval).
 *   --session <id>          Continue a specific session via --session-id; send only the delta brief.
 *   --resume-last           Rejected: muse resume --last is TUI-only. Use --session <id>.
 *   --timeout <dur>         Relay-side watchdog (default: off). Durations use h/m/s strings.
 *   --out-dir <dir>         Where to write run artifacts (default: a fresh dir under system temp).
 *   -h, --help              Show this help.
 *
 * Result: written to <out-dir>/result.json and summarized on stdout —
 *   status, exitCode, signal, museVersion, sessionId, finalMessage (from
 *   run.terminal.completed), touchedFiles, and paths to events.jsonl and final.txt.
 *
 * Exit codes: a pre-run usage error exits 2 before any run and writes no result
 * file; a missing `muse` binary exits 127; otherwise the exit code mirrors Muse's
 * own except a zero child exit without run.terminal.completed is reported failed.
 * Once the brief validates, result.json is written on every outcome —
 * completed, failed, timeout, aborted, or muse_unavailable.
 */
...
const MUSE_EFFORT = new Set(["none", "minimal", "low", "medium", "high", "xhigh", "ultra"]);
const MUSE_PROVIDER = new Set(["echo", "meta"]);

const IMPLEMENTER_KEY = "muse";
...
function parseArgs(argv) {
  ...
  if (opts.resumeLast) {
    fail("muse resume --last is TUI-only; headless resume is --session <id>");
  }
  if (opts.model !== null && !SAFE_MODEL.test(opts.model)) {
    fail("--model contains unsupported characters (allowed: letters, digits, . _ : / -)");
  }
  if (opts.effort !== null && !MUSE_EFFORT.has(opts.effort)) {
    fail(`invalid --effort "${opts.effort}" (expected: ${[...MUSE_EFFORT].join(", ")})`);
  }
  if (opts.provider !== null && !MUSE_PROVIDER.has(opts.provider)) {
    fail(`invalid --provider "${opts.provider}" (expected: echo or meta)`);
  }
  ...
  if (opts.session !== null && !SAFE_SESSION.test(opts.session)) {
    fail("--session must be a shell-safe session id (letters, digits, . _ : -)");
  }
  return opts;
}
...
function headerComment() {
  ...
  if (!match) return "relay.mjs — dispatch a brief to muse exec\n";
...
function museVersion(probeTimeoutMs) {
  try {
    const version = execFileSync("muse", ["--version"], {
      encoding: "utf8",
...
function buildArgv(opts, briefPath) {
  ...
  if (opts.effort) argv.push("--reasoning-effort", opts.effort);
  return argv;
}

function consumeMuseEvent(event, state) {
  if (!event || typeof event !== "object") return;
  const stream = event.stream;
  if (stream && stream.kind === "session" && typeof stream.id === "string" && stream.id) {
    state.sessionId = stream.id;
  }
  ...
  if (typeof payload.text === "string") state.finalMessage = payload.text;
  state.terminalOk = payload.terminal == null || payload.terminal === "completed";
}
...
function reportUnavailable(writeResult, resultPath) {
  const result = writeResult({ status: "muse_unavailable", exitCode: 127, signal: null, sessionId: null, finalMessage: "", touchedFiles: null });
  printSummary(result, resultPath);
  process.stderr.write("relay: `muse` not found on PATH. Install Muse Code and run `muse login`.\n");
  process.exit(127);
}
...
function reportVersionFailure(opts, writeResult, run, error, probeTimeoutMs) {
  ...
  const message = timedOut
    ? `muse --version preflight timed out after ${probeTimeoutMs}ms; Muse was not dispatched`
    : `muse --version preflight failed${Number.isInteger(error?.status) ? ` with exit ${error.status}` : ""}; Muse was not dispatched`;
...
function dispatchToMuse(opts, run, writeResult) {
  const argv = buildArgv(opts, run.briefPath);
  const child = spawn("muse", argv, {
    cwd: opts.cd,
  ...
  const scan = makeLineScanner((event) => consumeMuseEvent(event, state));
  ...
  const assembleFinal = () => {
    const message = (state.finalMessage || "").trim();
  ...
  for (const sig of ["SIGTERM", "SIGINT", "SIGHUP"]) {
    process.on(sig, () => {
      ...
      const abortedFields = {
        ...
        error: `the relay was killed by ${sig}; muse was terminated with it — inspect the working tree before re-dispatching`,
      };
  ...
  child.on("close", (code, signal) => {
    ...
    const terminalOk = state.sawTerminal && state.terminalOk;
    const succeeded = code === 0 && !watchdogFired && terminalOk;
    const result = writeResult({
      ...
      ...(watchdogFired ? { error: `muse did not finish within --timeout ${opts.timeout}; killed by the relay watchdog` } : {}),
      ...(!succeeded && !watchdogFired && code === 0 && !state.sawTerminal
        ? { error: "muse exited 0 without a run.terminal.completed event" }
        : {}),
    });
    printSummary(result, run.resultPath);
    process.exit(result.exitCode);
...
function main() {
  ...
  const probe = museVersion(probeTimeoutMs);
  ...
  dispatchToMuse(opts, run, writeResult);
}

function printSummary(result, resultPath) {
  const lines = [];
  lines.push("");
  lines.push(`relay: ${result.status} (exit ${result.exitCode}${result.signal ? `, killed by ${result.signal}` : ""})  ·  muse ${result.museVersion ?? "?"}`);
  if (result.signal === "SIGKILL" && result.status === "failed") lines.push("hint: the host killed the process (commonly the OOM killer or a supervisor timeout) — this is not a muse error; check host memory and re-dispatch, or split the task into smaller briefs.");
  if (result.signal === "SIGTERM" && result.status === "failed") lines.push("hint: something outside the relay terminated muse (a supervisor, the session ending, or a manual kill) — when the relay itself does the killing it reports status \"timeout\" or \"aborted\" instead; inspect the working tree before re-dispatching.");
