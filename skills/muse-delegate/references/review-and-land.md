...
Muse did the typing; you own the judgment. This is where delegation earns its keep or quietly ships
...
`result.json` carries Muse's own claim that the gates passed. Treat that as a claim, not evidence —
...
- **Scope creep** — did Muse change things the brief said to leave untouched? Unasked refactors,
...
- **Quiet judgment calls** — sometimes Muse makes a defensible decision the brief didn't anticipate.
...
- **Hardcoded success or fixture data** on a path the brief says does real work — a canned
  `{status: "ok"}` or default return passes tests *by design*. If Muse couldn't implement something,
...
Anything the sweep catches goes back to Muse as a delta brief (below) or gets fixed in the tree
...
write `exec` *can* write the working tree, but committing should be the act of the party that verified
...
If the review turns up problems, don't restate the entire brief. Continue the same Muse session with
just the correction. Headless Muse has no `--resume-last`; pass the `sessionId` from `result.json`:

```bash
echo "The fix is right, but the test mocks the DB session - use the real migrated fixture instead, and
drop the now-unused import." | node "<skill-dir>/scripts/relay.mjs" --session <id> --cd /path/to/repo
```

(`<skill-dir>` is this skill's install directory — see [dispatch-and-poll.md](dispatch-and-poll.md).)

`--session` keeps Muse's session context from the first run (and its model), so a short delta is
...
- **Report design decisions** Muse made, and any defensible-but-unrequested turns it took.
...
- **Stop and ask** if correct completion requires going beyond the brief — don't expand the mandate on
  your own. A scope change is the human's call, not yours or Muse's.
