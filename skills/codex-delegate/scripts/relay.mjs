...
function runVersionProbe(binary, args, timeoutMs, env, shell = process.platform === "win32") {
...
async function codexVersion(probeTimeoutMs, env) {
  ...
  // (git installs a real git.exe and must NOT get this flag — see gitTouchedFiles.)
  const probe = await runVersionProbe("codex", ["--version"], probeTimeoutMs, env);
...
async function main() {
  ...
  const probe = await codexVersion(probeTimeoutMs, env);
