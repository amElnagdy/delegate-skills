...
`kiro-delegate` (Kiro CLI); siblings like
...
| **implementer** | the separate agent (Claude, Cline, Codex, OpenCode, Antigravity, Grok, Kimi, Qoder, Vibe, Cursor, Pi, Oh My Pi, Aider, Copilot, Warp, ZCode, Command Code, Kiro) | "worker", "sub-agent", "executor" |
...
`aider` / `copilot` / `oz` / `omp` / `zcode` / `cmd` / `kiro-cli`) and the skill's `relay.mjs`.
