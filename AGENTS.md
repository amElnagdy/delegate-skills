...
and land the result. Nineteen implementer skills ship today: `claude-delegate` (Claude Code),
`cline-delegate` (Cline CLI), `codex-delegate` (OpenAI Codex), `opencode-delegate` (OpenCode),
`agy-delegate` (Google Antigravity), `grok-delegate` (Grok Build), `kilo-delegate` (Kilo),
`kimi-delegate` (Kimi Code), `muse-delegate` (Muse Code),
...
| **implementer** | the separate agent (Claude, Cline, Codex, OpenCode, Antigravity, Grok, Kilo, Kimi, Muse, Qoder, Vibe, Cursor, Pi, Oh My Pi, Aider, Copilot, Warp, ZCode, Command Code) | "worker", "sub-agent", "executor" |
...
| `run`, `agent` (`code`/`plan`), `session`, `--auto`, `--pure`, `--variant`, `--continue` | Kilo's own terms — use verbatim when discussing `kilo` | "sandbox" (Kilo has no sandbox enum; autonomy is the agent); never present `build` as a Kilo agent |
| `exec`, `--json`, `--prompt-file`, `--session-id`, `--disable-approval`, `--disable-write`, `--disable-shell`, `--trust-workspace`, `--user-input-auto-resolve`, `--reasoning-effort`, `--provider` (`echo`/`meta`) | Muse's own terms — use verbatim when discussing `muse` | never call `--yolo` the default or a permission mode; `muse resume` is TUI-only and is never invoked; `--provider` is relay-only, not a fleet dial |
...
`cline` / `codex` / `opencode` / `agy` / `grok` / `kilo` / `kimi` / `muse` / `qodercli` / `vibe` / `cursor-agent` / `pi` /
...
- If you touch how a `relay.mjs` launches its implementer CLI, smoke-test on Windows too (native
  ...
  Vibe relay launch are unverified. Muse's upstream launcher is UNIX-only.
