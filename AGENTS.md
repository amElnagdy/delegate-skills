...
| **implementer** | the separate agent (Claude, Cline, Codex, OpenCode, Antigravity, Grok, Kimi, Qoder, Vibe, Cursor, Pi) | "worker", "sub-agent", "executor" |
...
| `session`, `--json`, `-v` (verbose), `--cwd`, `--model`, `--provider`, `--id` (resume), `--plan`, `--timeout` | Cline's own terms — use verbatim when discussing `cline` | don't paraphrase them |
