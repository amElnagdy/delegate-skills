...
| [`hermes-delegate`](skills/hermes-delegate/SKILL.md) | [Hermes Agent](https://hermes-agent.nousresearch.com/docs) (`hermes`) | headless `chat -q -Q`; explicit `--yolo` on write runs; no sandbox or permission modes exist [^none] | `--read-only` (restricted `--toolsets`) — best-effort [^none] | `--resume-last`, `--resume <id>`, `--run-budget <seconds>` |
